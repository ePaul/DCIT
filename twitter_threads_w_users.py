...
# dict of mappings between tweet id's and their user id
...
#        self.offset = '\t' * self.parent_layers
        self.offset = str(self.parent_layers) 
    ...
    def __str__(self):
        ...
        # each subtree is wrapped in html wrappers
        if self.children :
            olist = ['<tweet depth=' + self.offset + ' id=' + str(self.id) + ' user=' + str(self.user) + ' text=\"' + self.text.encode("utf-8").replace('\n',' ').replace('"','%quot%') + '\">'] + [str(ch) for ch in self.children] + ['</tweet>']
        else :
            olist = ['<tweet depth=' + self.offset + ' id=' + str(self.id) + ' user=' + str(self.user) + ' text=\"' + self.text.encode("utf-8").replace('\n',' ').replace('"','%quot%') + '\"></tweet>']
        return '\n'.join(olist)
        # # string representation of a discussion thread is string representation
        ...
        # olist = [self.offset + str(self.id) + '\t' + str(self.user) + '\t' + self.text.encode("utf-8").replace('\n',' ')] + \
...
for ifile in args.files:
    for line in ifile:
	    ...
	    users[tweet_id] = tweet["user"]["id_str"]
            inreply_id = tweet["in_reply_to_status_id_str"]
...
print >> sys.stderr, "Total tweets processed: %d" % total
...
for dt in discussion_threads[:args.n_threads]:
    ...
    #    if dt.total < 15:
    #        continue
    print "<thread size=" + str(dt.total) + " max_depth=" + str(dt.layers) + ">"
    print dt
    print "</thread>"
print "</body>\n</html>"
